...
import { Plus, Filter, X, TrendingDown } from 'lucide-react';
...
export default function HomeScreen() {
  ...
  const { expenses, loading, refresh } = useExpenses(dateFilter, filterCategoryId || undefined, filterBankId || undefined);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pt-12 pb-4">
        ...
        {/* Filters dropdown */}
        {showFilters && (
          <div className="mt-3 card-glass rounded-2xl p-4 space-y-3">
            <div>
              <p className="text-xs font-medium text-gray-500 mb-2">Category</p>
              <div className="flex gap-2 flex-wrap">
                <button
                  onClick={() => setFilterCategoryId('')}
                  className={`px-3 py-1.5 rounded-xl text-xs font-medium ${!filterCategoryId ? 'bg-blue-500 text-white' : 'bg-white/70 text-gray-600 border border-gray-200'}`}
                >
                  All
                </button>
                {categories.map(cat => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all ${filterCategoryId === cat.id ? 'text-white' : 'bg-white/70 text-gray-600 border border-gray-200'}`}
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-medium ${!filterBankId ? 'bg-blue-500 text-white' : 'bg-white/70 text-gray-600 border border-gray-200'}`}
                  >
                    All
                  </button>
                  {accounts.map(acc => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl text-xs font-medium flex items-center gap-1 ${filterBankId === acc.id ? 'text-white' : 'bg-white/70 text-gray-600 border border-gray-200'}`}
      ...
      <div className="px-4 mb-4">
        <div className="rounded-3xl p-5 text-white overflow-hidden relative" style={{ background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)' }}>
          ...
          <p className="text-blue-200 text-sm font-medium mb-1">Total Spent</p>
          ...
          {byCategory.slice(0, 3).length > 0 && (
            <div className="flex gap-2 flex-wrap">
              {byCategory.slice(0, 3).map(cat => (
                <div key={cat.id} className="flex items-center gap-1.5 bg-white/10 rounded-full px-2.5 py-1">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="w-16 h-16 bg-gray-100 rounded-3xl flex items-center justify-center mb-3">
              <TrendingDown size={28} className="text-gray-400" />
            </div>
            <p className="text-gray-600 font-medium">No expenses</p>
      ...
      <div className="fixed bottom-24 right-4 z-40">
        <button
          ...
          className="btn-primary w-14 h-14 rounded-2xl flex items-center justify-center shadow-2xl"
        >
          <Plus size={24} />
        </button>
      </div>
